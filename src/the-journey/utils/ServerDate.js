...
// some adjustments to make it an importable module, 'var' > 'let', '==' > '===',
// and added serverNow() function that returns luxon DateTime
...
// Synchronize the ServerDate object with the server's clock.
  function synchronize() {
    ...
    function requestSample() {
      ...
      request.open("HEAD", URL + concatenationChar + "noCache=" + Date.now() + rand);
...
// a small amount.
  setInterval(function()
  {
    ...
    // direction.
    let delta = Math.max(-ServerDate.amortizationRate,
...
// util function wraps now in a luxon object
export function serverNow() {
  return DateTime.fromJSDate(new Date(ServerDate.now()))
}
